# This Makefile is for the Text::CSV::Euclid extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Text/CSV/Euclid.pm]
#     AUTHOR => q[Ryan C. Thompson <rct@thompsonclan.org>]
#     NAME => q[Text::CSV::Euclid]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], version=>q[0] }
#     VERSION_FROM => q[lib/Text/CSV/Euclid.pm]
#     clean => { FILES=>q[Text-CSV-Euclid-*] }
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = Text::CSV::Euclid
NAME_SYM = Text_CSV_Euclid
VERSION = 0.0.1
...
MAKE = make
FULLEXT = Text/CSV/Euclid
BASEEXT = Euclid
PARENT_NAME = Text::CSV
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Text/CSV/Euclid.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Text/CSV/Euclid.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Text/CSV
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Text/CSV
...
TO_INST_PM = lib/Text/CSV/Euclid.pm

PM_TO_BLIB = lib/Text/CSV/Euclid.pm \
	blib/lib/Text/CSV/Euclid.pm
...
DISTNAME = Text-CSV-Euclid
DISTVNAME = Text-CSV-Euclid-0.0.1
...
manifypods : pure_all  \
	lib/Text/CSV/Euclid.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Text/CSV/Euclid.pm $(INST_MAN3DIR)/Text::CSV::Euclid.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                Text-CSV-Euclid' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.0.1' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Provide a csv parser with options specified on the' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:              ' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Ryan C. Thompson <rct@thompsonclan.org>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Provide a csv parser with options specified on the</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Ryan C. Thompson &lt;rct@thompsonclan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Text/CSV/Euclid.pm blib/lib/Text/CSV/Euclid.pm 
